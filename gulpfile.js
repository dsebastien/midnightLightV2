...
gulp.task('check-js-style', 'Enforce JavaScript code style', function(){
  ...
  // Save modified files
  //.pipe(gulp.dest(appFolder + '/scripts'))
...
gulp.task('scripts-javascript', 'Transpile JavaScript (ES6 to ES5 using Babel) and generate sourcemaps', function(){
  ...
  .pipe($.changed(tempFolder + '/scripts', {
  extension : '.js'
  }))
...
gulp.task('serve', 'Watch files for changes and rebuild/reload automagically', [ 'prepare-serve' ], function(){
  ...
  gulp.watch([ appFolder + '/scripts/**/*.js' ], [ 'check-js-style', 'check-js-quality', 'scripts-javascript' ]); // JavaScript changes will force a reload
  gulp.watch([ appFolder + '/images/**/*' ], reload); // image changes will force a reload
});

gulp.task('prepare-serve', 'Do all the necessary preparatory work for the serve task', [ 'ts-lint', 'check-js-style', 'check-js-quality' ], function(){
  return runSequence([
  'gen-ts-refs',
  'scripts-javascript',
  'scripts-typescript',
